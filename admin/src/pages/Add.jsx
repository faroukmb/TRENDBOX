  ...
  const onSubmitHandler=async (e)=>{
     ...
     try {
      ...
      image4 && formData.append("image4",image4);
       const res = await axios.post("http://localhost:4000/api/product/add",formData,{headers : {token}});
       if (res.data.success){
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='w-full'>
        ...
        <input type="text" value={name} onChange={(e)=>setName(e.target.value)} className='w-full max-w-[500px] px-3 py-2' placeholder='Type here' />
      </div>
      <div className='w-full'>
        <p className='mb-2'>Product Description</p>
        <textarea type="text" onChange={(e)=>setDescription(e.target.value)}
        value={description} className='w-full max-w-[500px] px-3 py-2' placeholder='Write content here' />
      </div>
      <div className="flex flex-col sm:flex-row gap-2 w-full sm:gap-8">
      <div>
        <p className='mb-2 w-32'>Product category</p>
      ...
      <div>
        <p className='mb-2 w-40'>Product Sub category</p>
      ...
      <div className='flex gap-2 mt-2'>
        <input type="checkbox" onChange={()=> setBestseller(prev => !prev)} id='bestseller' checked={bestseller} />
        <label className='cursor-pointer' htmlFor="bestseller"> Add to bestseller
        </label>
      </div>
